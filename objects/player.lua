    ...
    return {
        ...
        movePlayer = function(self, dt)
            ...
            for index, bullet in pairs(self.bullets) do
                if (bullet.distance > Bullet_distance * love.graphics.getWidth()) and (bullet.exploading == 0) then
                    bullet:expload()
                end

                if bullet.exploading == 0 then
                    bullet:move()
                elseif bullet.exploading == 2 then
                    self.destroyBullet(self, index)
                end
            end
        end,
